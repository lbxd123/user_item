...
class TCNN(nn.Module):
    ...
    def __init__(self, n_user_features, n_item_features, n_time_features, user_df, item_df, time_df, dim):
        ...
        self.time_features = nn.Embedding(n_time_features, dim, max_norm=1)

        self.user_df = user_df
        self.item_df = item_df
        self.time_df = time_df

        total_neighbours = user_df.shape[1] + item_df.shape[1] + time_df.shape[1]
    ...
    def forward(self, u, i, t, Train=True):
        user_ids = torch.LongTensor(self.user_df.loc[u].values)
        item_ids = torch.LongTensor(self.item_df.loc[i].values)
        time_ids = torch.LongTensor(self.time_df.loc[t].values)

        # [batch_size, user_neighbours, dim]
        user_features = self.user_features(user_ids)
        # [batch_size, item_neighbours, dim]
        item_features = self.item_features(item_ids)
        # [batch_size, time_neighbours, dim]
        time_features = self.time_features(time_ids)
        # [batch_size, total_neighbours, dim]
        uv = torch.cat([user_features, item_features, time_features], dim=1)
...
def train(epochs=50, batchSize=512, lr=0.001, dim=128, eva_per_epochs=1, need_eva=True):
    ...
    train_triples, test_triples, user_df, item_df, time_df, n_user_features, n_item_features, n_time_features = \
        DataLoaderMovieLens.read_data_user_item_time_df()

    net = TCNN(n_user_features, n_item_features, n_time_features, user_df, item_df, time_df, dim)
